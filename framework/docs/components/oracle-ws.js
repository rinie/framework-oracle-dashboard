...
// If no message arrives within this window, assume the connection is dead
// and force a reconnect (handles proxies that don't propagate close frames).
...
function resetHeartbeat() {
  ...
  heartbeatTimer = setTimeout(() => {
    console.warn("[oracle-ws] Heartbeat timeout — forcing reconnect");
    ws?.close();
...
function connect() {
  ...
  ws.addEventListener("message", (event) => {
    ...
    if (typeof event.data === "string") {
      // JSON control message (ping, error, …)
      try {
        const msg = JSON.parse(event.data);
        if (msg.type === "ping") return; // heartbeat — timer already reset above
        if (msg.type === "error") {
